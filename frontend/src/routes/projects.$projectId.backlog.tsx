...
export const Route = createFileRoute("/projects/$projectId/backlog")({
  ...
  head: () => ({
    meta: [
      { title: "Backlog — Flightdeck" },
      {
        name: "description",
        content: "Search, filter and sort every ticket in the project backlog.",
      },
      { property: "og:title", content: "Backlog — Flightdeck" },
      {
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <NewTicketDialog projectId={projectId} trigger={<Button size="sm">New ticket</Button>} />
      </header>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-3">
        ...
        <span className="ml-auto text-xs text-muted-foreground">{rows.length} tickets</span>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <Table>
          ...
          <TableBody>
            {rows.map((t) => (
              <TableRow key={t.id} className="hover:bg-surface">
